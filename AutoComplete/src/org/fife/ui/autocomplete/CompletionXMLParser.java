	...
	private FunctionCompletion createFunctionCompletion() {
		...
		if (funcCompletionType != null) {
			try {
				...
				Class[] paramTypes = { CompletionProvider.class, String.class,
						String.class };
				Constructor c = clazz.getDeclaredConstructor(paramTypes);
				fc = (FunctionCompletion) c.newInstance(new Object[] {
						provider, name, returnType });
	...
	public void endElement(String uri, String localName, String qName) {
		...
		if ("keywords".equals(qName)) {
			...
			if ("keyword".equals(qName)) {
				...
				} else if ("StateMachine".equals(type)) {
					c = createOtherCompletion(); // TODO: Create own completion
				} else if ("State".equals(type)) {
					c = createOtherCompletion();
				} else if ("Transition".equals(type)) {
					c = createOtherCompletion();
				} else {
					throw new InternalError("Unexpected type: " + type + "\nRemember to add type in CompletionXMLParser");
				}
				completions.add(c);
				inKeyword = false;
			} else if (inKeyword) {
